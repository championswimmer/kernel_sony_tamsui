...
uint dhd_master_mode = FALSE;  //MTD-CONN-CD-PING_FILTER-00*
...
{
/*
 * MTD_CONN_EC_Tapioca_ICS-00516
 * Symptom: download speed slows down after pressing power key
 *
 * Power mode PM_MAX makes DUT enter power saving mode.
 * DUT sleeps and listens TIM of beacon frame in power saving mode.
 * If beacon has TIM, DUT wakes up and sends PS-POLL frame.
 * This mechanism doesn't work well in some AP like QA's Buffalo G54 Wi-Fi AP
 * So adjust power mode PM_FAST which is initial power mode.
 * Power mode PM_FAST doesn't enter power saving mode when DUT transmits packets,
 * but enter power saving mode when DUT is idle.
 * */
//MTD_CONN_EC_Tapioca_ICS-00516-	int power_mode = PM_FAST;
...
//MTD_CONN_EC_Tapioca_ICS-00516-]
                ...
                printk(KERN_INFO "%s: bcn_li_dtim = %d\n", __func__, bcn_li_dtim);
...
dhd_preinit_ioctls(dhd_pub_t *dhd)
{
...
#endif /* WLMEDIA_HTSF */
#ifdef PNO_SUPPORT
    DHD_ERROR(("%s: Fred Set PNO Event mask \n", __FUNCTION__));
	...
	//MTD-CONN-CD-PING_FILTER-00*[
	/* Setup filter to allow only unicast */
	dhd->pktfilter[0] = "100 0 0 0 0x01 0x01"; //"100 0 0 0 0x01 0x00";
	dhd->pktfilter[1] = "101 0 0 12 0xffff000000000000000000ff00000000000000000000ff 0x0800000000000000000000010000000000000000000008"; //Hugh 1020 //NULL;
	dhd->pktfilter[2] = NULL;
	dhd->pktfilter[3] = NULL;
	printk("[DHD]Hugh %s dhd->pktfilter[1]=%s\n", __FUNCTION__, dhd->pktfilter[1]);
	//MTD-CONN-CD-PING_FILTER-00*]
#if defined(SOFTAP)
	if (ap_fw_loaded) {
		int i;
...
dhd_module_init(void)
{
	...
	*   We pull power in msm7x2x_init() to avoid mmc driver failed to detect Wi-Fi chip.
	*/
