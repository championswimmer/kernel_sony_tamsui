	...
	u8 length, const void *values)
{
	...
	int num_bytes,count=0;/*FIH-MTD-PERIPHERAL-CH-CODE_DEFECT-00++*/
	int retval;
	struct cyttsp_i2c *ts = container_of(handle, struct cyttsp_i2c, ops);
	num_bytes = length;
	data[0] = addr;
	...
	memcpy(&data[1], values, count);/*FIH-MTD-PERIPHERAL-CH-CODE_DEFECT-01++*/
	num_bytes -= count;
	retval = i2c_master_send(ts->client, data, count+1);
	
	if (retval < 0)
	{
		printk(KERN_INFO "%s: 1st i2c_master_send retval: %d\n", __func__, retval);
		return retval;
	}		
	...
	while (num_bytes > 0) {
		...
		if (retval < 0)
		{
			printk(KERN_INFO "%s: 2nd i2c_master_send retval: %d\n", __func__, retval);
			return retval;
		}			
	}
	...
	const struct i2c_device_id *id)
{
	...
	int retval=0;/*FIH-MTD-PERIPHERAL-CH-CODE_DEFECT-00++*/
