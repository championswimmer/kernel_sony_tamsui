...
/*FIH-SW3-PERIPHERAL-AH-TouchDriver_Porting_JLO+[*/
#if 1
#define TP_RST_N_GPIO		121
#define TP_I2C_IRQ_GPIO 	((fih_get_product_phase() < Phase_DP) ? 40:86)
 ...
 */
static int synaptics_i2c_init(int on)
{ 
	int ret;
	...
	if (ret) 
	{
		printk(KERN_ERR"%s: [TOUCH] vreg get failed \n", __func__); 
		return -EFAULT;
	}
	...
	if (on) 
	{
		 ...
		 */
		printk(KERN_INFO"%s: [TOUCH] gpio_tlmm_config touch interrupt pin:%d \n", __func__, TP_I2C_IRQ_GPIO); 
		...
		if (ret)
		{
			printk(KERN_ERR "%s: [TOUCH] Failed to gpio_tlmm_config %d\n",
			       __func__, TP_I2C_IRQ_GPIO);
			return ret;
		}
		
		ret = gpio_request(TP_I2C_IRQ_GPIO, "TOUCH IRQ GPIO");
		if (ret) 
		{
			printk(KERN_ERR "%s: [TOUCH] Failed to request GPIO %d\n",
			       __func__, TP_I2C_IRQ_GPIO);
			return ret;
		}

		gpio_direction_input(TP_I2C_IRQ_GPIO);
		...
		{
			printk(KERN_ERR "%s: [TOUCH] Failed to gpio_tlmm_config %d\n",
			       __func__, TP_RST_N_GPIO);
			return ret;
		}
		
		ret = gpio_request(TP_RST_N_GPIO, "TOUCH RST GPIO");
		if (ret) 
		{
			printk(KERN_ERR "%s: [TOUCH] Failed to request GPIO %d\n",
			       __func__, TP_RST_N_GPIO);
		...
		//gpio_set_value_cansleep(TP_RST_N_GPIO, 1);

	} 
	else 
	{
		printk(KERN_INFO "[TOUCH] gpio_free(%d)\n", TP_I2C_IRQ_GPIO);
		gpio_free(TP_I2C_IRQ_GPIO);
	}
...
static int synaptics_poweron_reset(struct i2c_client *client)
{
	...
	/*FIH-SW3-PERIPHERAL-AH-TouchDriver_Porting_ICS-*/
	#if 0
	...
	gpio_set_value_cansleep(TP_RST_N_GPIO, LOW);
	mdelay(20);
	gpio_set_value_cansleep(TP_RST_N_GPIO, HIGH);
	mdelay(250);
...
ts_init_f19_data:	
	...
	for (i=0; i<10; i++)
	{
		...
		{
			mdelay(20);
			if (gpio_get_value(TP_I2C_IRQ_GPIO) == HIGH)
			{	
...
//	int buf_len = idx_palm;
	...
	if((gpio_get_value(TP_I2C_IRQ_GPIO) == LOW) || continue_read)
	{
				...
				else
				{
					...
					{	
						...
						{	
							...
							input_sync(ts->key_input_dev);/*FIH-SW3-PERIPHERAL-AH-TouchDriver_Porting_ICS+*/
						}	
...
//				}
	...
	{
		ret= gpio_get_value(TP_I2C_IRQ_GPIO);
	...
	struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	#else
		if (synaptics_i2c_init(1))
		{
			ERR_MSG("synaptics_i2c_init failed \n");
	...
	ret = synaptics_poweron_reset(ts->client);
	if (ret)
	{
		ERR_MSG("synaptics_poweron_reset failed \n");
		goto err_power_failed;
	}		
...
// input_allocate_device - key
	...
	printk(KERN_INFO "[TOUCH] synaptics_ts_probe: max_x %d, max_y %d\n", max_x, max_y);
	...
	input_set_abs_params(ts->input_dev, ABS_MT_PRESSURE, 0, 255, 0, 0); /*FIH-SW3-PERIPHERAL-AH-TouchDriver_Porting_ICS+ */
	#if 0
...
// Register input device - touch 
	...
	if (ret) {
		printk(KERN_ERR "[TOUCH] synaptics_ts_probe: Unable to register %s touch input device\n", ts->input_dev->name);
...
// Register input device - keys
	...
	if (ret) {
		printk(KERN_ERR "[TOUCH] synaptics_ts_probe: Unable to register %s key input device\n", ts->input_dev->name);
		goto err_input_register_device_failed;
	}
	
	client->irq = MSM_GPIO_TO_INT(TP_I2C_IRQ_GPIO);
