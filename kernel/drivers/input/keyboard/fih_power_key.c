	...
	case KEY_END:
		if( key == KEY_POWER || key == KEY_END )
		{
			...
			printk( "PKEY : Suspend %s, "
			       ...
			       jiffies_to_msecs( jiffies ), power_key_data->interval, jiffies_to_msecs( counter ) );

			if( !power_key_data->set_interval && ( suspend_lock || counter < msecs_to_jiffies( power_key_data->interval ) ) )
			{
