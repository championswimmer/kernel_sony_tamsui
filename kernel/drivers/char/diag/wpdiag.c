...
#include <linux/slab.h> //+WP5-HH_ON_DEVICE_QXDM-01
...
static long wp_diag_device_op_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	//printk(KERN_INFO "wp_diag: %s\n", __FUNCTION__);
	return 0;
}
...
//SW2-5-1-MP-DbgCfgTool-03*[
    ...
    //int boot_mode;  //WP5-HH_MODIFY_TEMPORARILY_&_ERICLIU_WILL_MODIFY_LATER-01
    ...
    //boot_mode = fih_read_boot_mode_from_smem(); //-WP5-HH_MODIFY_TEMPORARILY_&_ERICLIU_WILL_MODIFY_LATER-01

	//if (boot_mode != APPS_MODE_FTM) { //-WP5-HH_MODIFY_TEMPORARILY_&_ERICLIU_WILL_MODIFY_LATER-01
	if (1) { //+WP5-HH_MODIFY_TEMPORARILY_&_ERICLIU_WILL_MODIFY_LATER-01
		rc = DbgCfgGetByBit(DEBUG_MODEM_LOGGER_CFG, (int*)&cfg_val);
		//if ((rc == 0) && (cfg_val == 1) && (boot_mode != APPS_MODE_RECOVERY)) { //-WP5-HH_MODIFY_TEMPORARILY_&_ERICLIU_WILL_MODIFY_LATER-01
		if ((rc == 0) && (cfg_val == 1)) { //+WP5-HH_MODIFY_TEMPORARILY_&_ERICLIU_WILL_MODIFY_LATER-01
#ifdef DYNAMIC_ALLOC
...
const struct file_operations wp_diag_device_ops = {
	...
	.unlocked_ioctl		= wp_diag_device_op_ioctl,
