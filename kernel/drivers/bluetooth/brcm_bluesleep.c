...
#else
#undef BT_DBG
#define BT_DBG(fmt, arg...) printk("[BLUESLEEP_DBG] %s: "fmt"\n", __FUNCTION__, ## arg)
#endif
...
static void bluesleep_outgoing_data(void)
{
	...
	if (IDLE == gpio_get_value(bsi->ext_wake)) {
		BT_DBG("tx was sleeping");
		gpio_set_value(bsi->ext_wake, WAKE);
	}
...
static void bluesleep_tx_timer_expire(unsigned long data)
{
	...
	gpio_set_value(bsi->ext_wake, IDLE);
...
static int __init bluesleep_probe(struct platform_device *pdev)
{
	...
	/* assert bt wake */
	ret = gpio_direction_output(bsi->ext_wake, WAKE);
	if (ret)
