...
void __force __iomem *msm_mlog_base = NULL;
...
{
	msm_mlog_base = ioremap(MTD_MLOG_ADDR, MTD_MLOG_SIZE);
	
	if (msm_mlog_base == NULL)
	{
		printk(KERN_ERR "failed to map MLOG memory\n");
		return -ENOMEM;
	}
	
	printk(KERN_INFO "%s: msm_mlog_base = 0x%x\n", __func__, (unsigned int)msm_mlog_base);
	
	proc_create("mlog", S_IRUSR | S_IRGRP | S_IROTH, NULL, &proc_mlog_operations);
	return 0;
}
