...
#include "timer.h"
#include "include/mach/oem_rapi_client.h"

/*20120420 FelexChing WiFi_LDO_EN*/
#ifdef CONFIG_MSMHW_PDP_BOARD
//#define GPIO_WLAN_3V3_EN 119
#define GPIO_WLAN_3V3_EN 11
#else
...
static unsigned int msm_AR600X_setup_power(bool on)
{
	...
	if (rc) {
		pr_err("%s gpio_tlmm_config 11 failed,error = %d\n",
			__func__, rc);
		goto reg_disable;
	}
...
/*static int isl29021123_rpc(int threshold, uint32_t event)
{
