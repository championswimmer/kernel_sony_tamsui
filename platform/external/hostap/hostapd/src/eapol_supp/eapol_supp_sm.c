...
SM_STATE(SUPP_BE, SUCCESS)
{
	...
	if (FALSE == sm->is_simaka) {
		if (eap_key_available(sm->eap)) {
			/* New key received - clear IEEE 802.1X EAPOL-Key
			 * replay counter */
...
static void eapol_sm_txSuppRsp(struct eapol_sm *sm)
{
	...
	if (FALSE == sm->is_simaka) {
		...
		proxy_resp = eap_proxy_get_eapRespData(sm->eap_proxy,
						       &resp_len);
		if (proxy_resp == NULL) {
			wpa_printf(MSG_WARNING, "EAPOL: txSuppRsp -"
				   " EAP response data not available");
			    ...
			    const struct eapol_config *conf)
{
	...
	if (config && eap_allowed_method(sm->eap, EAP_VENDOR_IETF,
					 EAP_TYPE_SIM)) {
		sm->is_simaka = TRUE;
		if (sm->eap_proxy)
			sm->eap_proxy->eap_type = EAP_TYPE_SIM;
	} else if (config && eap_allowed_method(sm->eap, EAP_VENDOR_IETF,
						EAP_TYPE_AKA)) {
		sm->is_simaka = TRUE;
		if (sm->eap_proxy)
...
#ifdef SIM_AKA_QUALCOMM
	/* initialize the simaka flag before it enters the states */
	sm->is_simaka = FALSE;
	/* Initialize Eap Proxy */
	sm->eap_proxy = eap_proxy_init(sm, &eapol_cb);
...
#ifdef SIM_AKA_QUALCOMM
	/* EAP_SIM_AKA -> call EAP Proxy in case of SIM/AKA */
