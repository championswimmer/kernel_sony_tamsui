		   ...
		   unsigned char *recv_buf, unsigned long *recv_len)
{
	...
	buf_len = 0;
	do {
		/* wait for data on the socket, 2 sec timeout */
		tv.tv_sec = 2;
		tv.tv_usec = 0;
		FD_ZERO(&rws);
		   ...
		   unsigned char *sres, unsigned char *kc)
{
	...
	wpa_printf(MSG_WARNING, "SCARD: GSM auth - RAND= %x %x %x %x", _rand[0], _rand[1], _rand[2], _rand[3]);
	if (scard->sim_type == SCARD_GSM_SIM) {
		cmdlen = 5 + 16;
	...
	wpa_printf(MSG_WARNING, "SCARD: GSM auth - SRES= %x %x %x %x", sres[0], sres[1], sres[2], sres[3]);
	wpa_printf(MSG_WARNING, "SCARD: GSM auth - Kc= %x %x %x %x %x %x %x %x", kc[0], kc[1], kc[2], kc[3], kc[4], kc[5], kc[6], kc[7]);
